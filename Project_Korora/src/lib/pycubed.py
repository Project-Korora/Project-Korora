...
class Satellite:
    ...
    def __init__(self):
        ...
        try:
            self.radio1 = pycubed_rfm9x.RFM9x(
    ...
    def new_file(self, substring, binary=False):
        ...
        if self.hardware["SDcard"]:
            ...
            print(
                f"Creating new file in directory: /sd{_folder} "
                "with file prefix: {_file}"
            )
            try:
                chdir("/sd" + _folder)
            except OSError:
            ...
            for i in range(0xFFFF):
                ...
                if num is None:
                    continue
                ff = f"/sd{_folder}{_file}{num:05}.txt"
                if not stat.path.exists(ff):
                    n = num
    ...
    def burn(self, burn_num, dutycycle=0, freq=1000, duration=1):
        ...
        burn_num:  (string) which burn wire circuit to operate, either '1' or '2'
        ...
        print(
            f"\tFrequency of: {freq}Hz\n\tDuty cycle of: {(100*dtycycl/0xFFFF)}% "
