...
__author__ = "Martin F N Cooper, Edits by Rm91bmQgbWUh"
__version__ = "1.0.3"

from micropython import const

# Addressing
ALEN = const(6)  # Basic address length (callsign only)
AXLEN = const(7)  # Extended address length (with SSID)
OFF_DST = const(0)  # Offset to destination address
OFF_SRC = const(7)  # Offset to source address
OFF_VIA = const(14)  # Offset to 'via' fields

# SSID byte
HDLC_AEB = const(0x01)  # HDLC address extension bit
SSID = const(0x1E)  # SSID mask
CMDRESP = const(0x80)  # Command / response
REPEATED = const(0x80)  # Has been repeated
RESERVED = const(0x60)  # Reserved bits
SSSID_SPARE = const(0x40)  # EAX25
ESSID_SPARE = const(0x20)  # DAMA

# Frame control byte
PF = const(0x10)  # P/F
NR = const(0xE0)  # N(R) mask
NRS = const(5)  # N(R) shift
NS = const(0x0E)  # N(S) mask
NSS = const(1)  # N(S) shift
...
class FrameType:
    ...
    Lightweight MicroPython-compatible replacement for Enum-based FrameType.
    """

    _instances = {}

    def __new__(cls, value, name=None):
        # Lookup mode: FrameType(value)
        if name is None:
    ...
    def __repr__(self):
        return "FrameType.{}".format(self.name)
    ...
    def is_U(self):  # noqa: N802
        """Is this a U frame type?"""
        return (self.value & 0x03) == 0x03


# Define frame types (singletons)
FrameType.RR = FrameType(0x01, "RR")
"""Receive Ready"""
FrameType.RNR = FrameType(0x05, "RNR")
"""Receive Not Ready"""
FrameType.REJ = FrameType(0x09, "REJ")
"""Reject"""
FrameType.SREJ = FrameType(0x0D, "SREJ")
"""Selective Reject"""
FrameType.UI = FrameType(0x03, "UI")
"""Unnumbered Information"""
FrameType.DM = FrameType(0x0F, "DM")
"""Disconnect Mode"""
FrameType.SABM = FrameType(0x2F, "SABM")
"""Set Async Balanced Mode"""
FrameType.DISC = FrameType(0x43, "DISC")
"""Disconnect"""
FrameType.UA = FrameType(0x63, "UA")
"""Unnumbered Acknowledge"""
FrameType.SABME = FrameType(0x6F, "SABME")
"""Set Async Balanced Mode Extended"""
FrameType.FRMR = FrameType(0x87, "FRMR")
"""Frame Reject"""
FrameType.XID = FrameType(0xAF, "XID")
"""Exchange Identification"""
FrameType.TEST = FrameType(0xE3, "TEST")
"""Test"""
FrameType.UNK = FrameType(0xFF, "UNK")
"""Unknown frame type"""
FrameType.I = FrameType(0x00, "I")
"""I Frame"""
FrameType.S = FrameType(0x01, "S")
"""S Frame"""
FrameType.U = FrameType(0x03, "U")
"""U Frame"""

# Legacy Code with Enum, for reference.
...
class Address:
    ...
    def __init__(self, call, ssid=0, repeater=False):
        ...
        if call.endswith("*"):
            if repeater:
                ...
                raise TypeError("Cannot set has_been_repeated on non-repeater address")
        parts = call.split("-")
    ...
    def __repr__(self):
        return ("Address(call: {}, ssid: {}, repeater: {}, {}: {})").format(
            ...
            self._ssid,
            self._repeater,
            ("has_been_repeated" if self._repeater else "command_response"),
            (self._has_been_repeated if self._repeater else self._command_response),
        )
    ...
    def has_been_repeated(self):
        ...
        if not self._repeater:
            raise TypeError("has_been_repeated not valid for non-repeater address")
    ...
    def command_response(self):
        ...
        """
        if self._repeater:
            raise TypeError("command_response not valid for repeater address")
...
class Control:
    ...
    def __init__(self, frame_type, poll_final=False, recv_seqno=0, send_seqno=0):
...
class Frame:
    ...
    def unpack(cls, buffer):
        ...
        if not end:
            while not end:
                via.append(Address.unpack(buffer[offset : offset + AXLEN], True))
