...
VR3X = b"\xff\x00\xff\x00\xff\x00\xff\x00\x00\x00\xff\xff\xff\x00\xff\x00\x00\x00\xff\
        \x00\xff\xff\xff\x00\xff\x00\xff\x00\xff\x00\x00\x00\xff\x00\x00\x00\xff\xff\
        \xff\x00\x00\x00\xff\x00\xff\x00\xff\x00\x00\x00\xff"
...
class RFM9x:
    ...
    or "reliable datagram" delivery are supported however due to the limitations noted,
    "reliable datagram" is still subject to missed packets but with it,
    ...
    def tx_done(self):
        """Transmit status"""
        # if self.dio0:
        #     print('TxDIO0: {}, {}'.format(self.dio0.value,
        #            hex((self._read_u8(_RH_RF95_REG_12_IRQ_FLAGS) & 0x8) >> 3)))
    ...
    def rx_done(self):
        """Receive status"""
        if self.dio0:
            # print('RxDIO0: {}, {}'.format(self.dio0.value,
            #        hex((self._read_u8(_RH_RF95_REG_12_IRQ_FLAGS) & 0x40) >> 6)))
            return self.dio0.value
        else:
    ...
    def send(
        ...
        *,
        ...
        listening automatically after the packet is sent.
        ...
        if data == b"!":
            ...
            payload = self.buffview[:data_length]
        ...
        self._write_u8(_RH_RF95_REG_22_PAYLOAD_LENGTH, data_length)
    ...
    def receive(
        ...
        *,
        ...
        listening mode after reception of a packet, otherwise it will fall back to idle
        mode and ignore any future reception.
        All packets must have a 4-byte header for compatibilty with the
        RadioHead library.
        The header consists of 4 bytes (To,From,ID,Flags).
        The default setting will strip the header before returning the packet to the
        caller. If with_header is True then the 4 byte header will be returned with the
        packet. The payload then begins at packet[4].
        ...
        if not timed_out:
            if self.enable_crc and self.crc_error():
                ...
                # RadioHead header and at least one byte of data
                # -- reject this packet and ignore it.
                ...
                if fifo_length < 5:
                    ...
                    # send ACK unless this was an ACK or a broadcast
                        ...
                        # delay before sending Ack to allow receiver to get ready
                        ...
                        # # reject Retries if we've seen this idetifier from this
                        # # source before
                        # if (self.seen_ids[packet[1]] == packet[2]) and (
                        #     packet[3] & _RH_FLAGS_RETRY
                        # ):
                        #     print('duplicate identifier from this source.
                        #            rejecting...')
                        #     packet = None
    ...
    def receive_all(self, only_for_me=True, debug=False):
        ...
        if fifo_length > 0:
            ...
            if packetindex:
                ...
                # msg.append(bytes(
                #           self.buffview[packetindex[-1]:current_addr+fifo_length]))
