...
class ManagedResource:
    """
    Manages a singleton resource with your functions that initialize a resource
    and clean it up between uses.

    This class vends access to `resource` via a fair queue.
    Intended use is with something like a busio.SPI with on_acquire setting a chip
    select pin and on_release resetting that pin.
    ...
    def __init__(
        ...
        :param on_acquire: function(*args, **kwargs) => void
                            acquires your singleton resource (CS pin low or something)
        :param on_release: function(*args, **kwargs) => void
                            releases your singleton resource (CS pin high or something)
        """
    ...
    async def _aenter(self, args, kwargs):
        if self._owned:
            ...
            # This leverages the suspend() feature in tasko;
            # this current coroutine is not considered again until the owning job is
            # complete and __aexit__s below.
            # This keeps waiting handles as cheap as possible.
            await await_handle
        self._owned = True
    ...
    async def _aexit(self, args, kwargs):
        ...
        if len(self._ownership_queue) > 0:
            ...
            # By not resetting to unowned here we avoid unfair resource starvation in
