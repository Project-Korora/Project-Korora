    ...
    async def _run_at_fixed_rate(self):
        ...
        try:
            ...
            while True:
                ...
                # Try to reschedule for the next window without skew.
                # If we're falling behind, just go as fast as possible
                # & schedule to run "now." If we catch back up again we'll return
                # to seconds_per_invocation without doing a bunch of catchup runs.
...
class Loop:
    ...
    def add_task(self, awaitable_task, priority):
        """
        Add a concurrent task
        (known as a coroutine, implemented as a generator in CircuitPython)
        Use:
    ...
    async def sleep(self, seconds):
        ...
        :param seconds: Floating point;
                        will wait at least this long to call your task again.
        """
    ...
    def suspend(self):
        """
        For making library functions that suspend
        and then resume later on some condition
        E.g., a scope manager for SPI

        To use this you will stash the resumer somewhere to call from another coroutine
        , AND you will `await suspender` to pause this stack at the spot you choose.
    ...
    def schedule(self, hz: float, coroutine_function, priority, *args, **kwargs):
        ...
        async def main_loop:
            await your_code()
                scheduled_task = get_loop().schedule(hz=100,
                                                    coroutine_function=main_loop)
            get_loop().run()

        :param hz: How many times per second should the function run?
        :param coroutine_function: the async def function you want invoked on your
                                    schedule
        :param event_loop: An event loop that can .sleep() and .add_task.
                            Like BudgetEventLoop.
        """
    ...
    def run(self):
        ...
        (in CircuitPython).
        Other Exceptions that reach the runner break out, stopping your app and showing
        a stack trace.
        """
    ...
    def _step(self):
        ...
        # Run the ready tasks,
        # simultaneously removing them from the _sleeping and _ready lists
        ...
        if len(self._tasks) == 0 and len(self._sleeping) > 0:
            ...
            if sleep_nanos > 0:
                ...
                # If/when interrupts are implemented this will likely need to change.
    ...
    def _run_task(self, task: Task):
        """
        Runs a task and re-queues for the next loop if it is both
        (1) not complete and (2) not sleeping.
        """
        ...
        try:
            ...
            # If a sleeping task re-suspends it will have already put itself in the
            # sleeping queue.
    ...
    async def _sleep_until_nanos(self, target_run_nanos):
        ...
        self._current = None
        # Pretty subtle here.  This yields once, then it continues next time the task
