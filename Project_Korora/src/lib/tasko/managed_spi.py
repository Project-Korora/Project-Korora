...
class ManagedSpi:
    ...
    def cs_handle(self, chip_select):
        ...
        You can read or write batches of data from an sd card while sending updates to
        a display between read batches, while reading sensor data from 3 different
        sensors on a timer on a single spi bus - without coordinating between them.
        Only await the handle for each task's turn with the bus.
