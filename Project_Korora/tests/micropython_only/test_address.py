...
import Project_Korora.src.lib.ax25 as ax25


@pytest.mark.parametrize(
    "in_call, in_base_only, result",
    [
        ("W1AW", False, True),
        ("W1AW-0", False, True),
        ("W1AW-1", False, True),
...
@pytest.mark.parametrize(
    "in_call, call, ssid",
    [("W1AW", "W1AW", 0), ("W1AW-0", "W1AW", 0), ("W1AW-1", "W1AW", 1)],
)
...
@pytest.mark.parametrize(
    "in_call, in_ssid, call, ssid", [("W1AW", 1, "W1AW", 1), ("W1AW-1", 1, "W1AW", 1)]
)
...
@pytest.mark.parametrize(
    "in_call", [("W1-AW-AW"), ("W1!AW"), ("W1AWAWAW"), ("W1AW-32"), ("W1AW-ABC")]
)
...
@pytest.mark.parametrize(
    "in_call, call", [("W1AW", "W1AW"), ("W1AW-0", "W1AW"), ("W1AW-1", "W1AW-1")]
)
...
@pytest.mark.parametrize(
    "in_call, in_ssid, call",
    [("W1AW", 0, "W1AW"), ("W1AW", 1, "W1AW-1"), ("W1AW-1", 1, "W1AW-1")],
)
def test_str_callsign_and_ssid(in_call, in_ssid, call):
    assert str(ax25.Address(in_call, in_ssid)) == call


@pytest.mark.parametrize("in_call, call", [("W1AW*", "W1AW*"), ("W1AW-2*", "W1AW-2*")])
...
def test_repr(
    in_call, in_ssid, in_repeater, in_hbr, in_cr, rep_call, rep_ssid, rep_hbr, rep_cr
):
...
@pytest.mark.parametrize(
    "in_call, in_repeater, repeater", [("W1AW", False, False), ("W1AW", True, True)]
)
...
@pytest.mark.parametrize(
    "in_call, has_been_repeated", [("W1AW", False), ("W1AW*", True)]
)
...
@pytest.mark.parametrize(
    "in_call, before, repeated, after",
    [("W1AW", False, True, True), ("W1AW*", True, False, False)],
)
...
@pytest.mark.parametrize(
    "in_call, in_ssid, packed_call, last_byte",
    [
        ("W1AW", 0, b"\xae\x62\x82\xae\x40\x40", 0x60),
        ("W1AW", 1, b"\xae\x62\x82\xae\x40\x40", 0x62),
        ("W1AW", 15, b"\xae\x62\x82\xae\x40\x40", 0x7E),
...
@pytest.mark.parametrize(
    "in_call, in_cmdresp, packed_call, last_byte",
    [
