...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ax25.Address("WR6ABD-5"), ax25.Address("W1AW"), "WR6ABD", 5, "W1AW", 0),
        (ax25.Address("WR6ABD-0"), ax25.Address("W1AW-3"), "WR6ABD", 0, "W1AW", 3),
        (ax25.Address("WR6ABD-5"), "W1AW", "WR6ABD", 5, "W1AW", 0),
        ("WR6ABD-5", ax25.Address("W1AW"), "WR6ABD", 5, "W1AW", 0),
    ],
)
def test_construct_addresses(in_dst, in_src, dst_call, dst_ssid, src_call, src_ssid):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            0x00,
            None,
            b"\xae\x62\x82\xae\x40\x40\x60" b"\xae\xa4\x6c\x82\x84\x88\x61" b"\x01",
        ),
        (
            ax25.FrameType.UI,
            "W1AW",
...
def test_pack(in_ft, in_dst, in_src, in_via, in_pid, in_data, expected):
    ctl = ax25.Control(in_ft)
    f = ax25.Frame(in_dst, in_src, control=ctl, via=in_via, pid=in_pid, data=in_data)
    packed = f.pack()
...
@pytest.mark.parametrize(
    ...
    [
        (
            b"\xae\x62\x82\xae\x40\x40\x00" b"\xae\xa4\x6c\x82\x84\x88\x01" b"\x01",
            ax25.FrameType.RR,
            "W1AW",
            "WR6ABD",
            None,
