...
class TestLoop(TestCase):
    ...
    def test_schedule_rate(self):
        # Checks a bunch of scheduled tasks to make sure they hit their target fixed
        # rate schedule.
        # Pathological scheduling sees these tasks barge in front of others all the
        # time. Many run at a fairly high frequency.
        #
        # 98 tasks => 15khz throughput tested.
        #   Works reliably on macbook pro; microcontroller throughput will be..
        #   somewhat less.

        loop = Loop(debug=False)
        # Seconds to run the scheduler. (try with 10s if you suspect scheduler drift)
        duration = 1
        ...
        expected_tps = 0
        actual_tps = 0
        # Assert that all the tasks hit their scheduled count,
        # at least within +-5 iterations.
